...
void*
    ...
    {
		// puts("3333");
		// log__printf(NULL, MOSQ_LOG_INFO, "thread_worker1 start");
		// char *ip = "127.0.0.1";
  		int port = 1883;
        ...
        if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0){  
			// log__printf(NULL, MOSQ_LOG_INFO, "[-]Error in socket");
...
// #endif
	...
	puts("111");
	pthread_t tid1;
    if(!pthread_create(&tid1,NULL,thread_worker1,argv[1]))
	puts("fuck");
	// log__printf(NULL, MOSQ_LOG_INFO, "pthread_create return code: %d", te);
	puts("222");
	...
	rc = 0;
	log__printf(NULL, MOSQ_LOG_INFO, "normal rc: %d", rc);
