...
namespace Charon.Core.OS.Linux.Agent.Monitors
{
	...
	public class LinuxDebugger : Charon.Core.Agent.Monitor
	{
...
";
		...
		string FindExploitable()
		{
			...
			throw new CharonException("Error, LinuxDebugger could not find '" + target + "' in search path.");
		}
		...
		void _Start()
		{
			...
			{
				_procHandler = null;
				throw new CharonException("Could not start debugger '" + GdbPath + "'.  " + ex.Message + ".", ex);
			}
			...
			if (!File.Exists(_gdbPid) && _procHandler.HasExited)
				throw new CharonException("GDB was unable to start '" + Executable + "'.");
		...
		{
			string asan_log_path = Charon.Core.Runtime.SHARE.pathAsanReport + "." + strPid;
