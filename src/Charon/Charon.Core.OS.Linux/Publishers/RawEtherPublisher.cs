		...
		private UnixStream OpenSocket(uint? mtu)
		{
			...
			try
			{
				...
				if (mtu != null && ifr.ifru_mtu != mtu.Value)
					throw new CharonException("MTU change did not take effect.");
			...
			catch (InvalidOperationException ex)
			{
				if (ex.InnerException != null)
				{
					...
					if (inner != null && inner.ErrorCode == Errno.EPERM)
						throw new CharonException("Access denied when opening the raw ethernet publisher.  Ensure the user has the appropriate permissions.", ex);
				}
