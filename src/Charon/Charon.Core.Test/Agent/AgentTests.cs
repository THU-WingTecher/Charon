...
using Charon.Core;
using Charon.Core.Agent.Channels;
...
namespace Charon.Core.Test.Agent
{
	[TestFixture]
	public class AgentTests
	{
		...
		public void SetUp()
		{
			si = SingleInstance.CreateInstance("Charon.Core.Test.Agent.AgentTests");
			si.Lock();
		}
		...
		public class AgentKillerPublisher : Charon.Core.Publisher
		{
		...
		public void StartAgent()
		{
			...
			{
				process.StartInfo.FileName = "Charon.exe";
			...
			else
			{
				...
				string Charon = "Charon.exe";
				foreach (var dir in paths)
				{
					var candidate = Path.Combine(dir, Charon);
				...
				process.StartInfo.Arguments = "--debug {0} -a tcp".Fmt(Charon);
			}
...
{0}
	...
	<Test name='Default' replayEnabled='false'>
		...
		public class LoggingMonitor : Charon.Core.Agent.Monitor
		{
...
<Charon>
	...
	<StateModel name='TheState' initialState='Initial'>
		<State name='Initial'>
			...
			<Action type='call' method='Foo' publisher='Charon.Agent'/>
		</State>
	</StateModel>
