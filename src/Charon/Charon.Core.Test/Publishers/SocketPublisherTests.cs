...
";
		...
		public void MulticastUdp6Test()
		{
			...
			try
			{
				...
				PitParser parser = new PitParser();
				Dom.Dom dom = parser.asParser(null, new MemoryStream(ASCIIEncoding.ASCII.GetBytes(xml)));
				Charon.Core.Publishers.UdpPublisher pub = dom.tests[0].publishers[0] as Charon.Core.Publishers.UdpPublisher;
				pub.Interface = local; 
		...
		{
			// If the data model is too large, the publisher should throw a CharonException
			...
			Engine e = new Engine(null);
			Assert.Throws<CharonException>(delegate() { e.startFuzzing(dom, config); });
		}
		...
		[Test, ExpectedException(typeof(CharonException), ExpectedMessage = "The resolved IP '127.0.0.1:0' for host '127.0.0.1' is not compatible with the RawV6 publisher.")]
...
";
		...
		[Test, ExpectedException(typeof(CharonException), ExpectedMessage = "Could not resolve scope id for interface with address 'fe80::'.")]
