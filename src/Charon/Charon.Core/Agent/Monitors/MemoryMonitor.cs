...
namespace Charon.Core.Agent.Monitors
{
	...
	public class MemoryMonitor : Charon.Core.Agent.Monitor
	{
			...
			: base(agent, name, args)
		{
			...
			if (!Pid.HasValue && string.IsNullOrEmpty(ProcessName))
				throw new CharonException("Either pid or process name is required.");

			if (Pid.HasValue && !string.IsNullOrEmpty(ProcessName))
				throw new CharonException("Only specify pid or process name, not both.");
		}
