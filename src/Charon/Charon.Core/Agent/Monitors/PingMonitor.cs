...
namespace Charon.Core.Agent.Monitors
{
	...
	public class PingMonitor : Charon.Core.Agent.Monitor
	{
			...
			: base(agent, name, args)
		{
			...
			if (!hasPermissions)
				throw new CharonException("Unable to open ICMP socket.  Ensure user has appropriate permissions.");
			...
			if (Platform.GetOS() != Platform.OS.Windows)
			{
				...
				if (len > maxLen)
					throw new CharonException("Error, the value of parameter 'Data' is longer than the maximum length of " + maxLen + ".");
			}
		}
