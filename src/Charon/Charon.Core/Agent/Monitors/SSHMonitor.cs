...
namespace Charon.Core.Agent.Monitors
{
	...
	public class SSHMonitor : Charon.Core.Agent.Monitor
	{
			...
			: base(agent, name, args)
		{
			...
			if (Password == null && KeyPath == null)
				throw new CharonException("Either Password or KeyPath is required.");
			...
			{
				throw new CharonException("'CheckValue' is not a valid regular expression.  " + ex.Message, ex);
			}
		}
		...
		void OpenClient(ConnectionInfo ci)
		{
			...
			catch (Exception ex)
			{
				...
				throw new CharonException("Could not start the ssh monitor.  " + ex.Message, ex);
			}
		}
