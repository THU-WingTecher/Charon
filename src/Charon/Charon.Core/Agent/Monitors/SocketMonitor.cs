...
namespace Charon.Core.Agent.Monitors
{
	...
	public class SocketMonitor : Charon.Core.Agent.Monitor
	{
			...
			: base(agent, name, args)
		{
			...
			if (Host != null)
			{
				...
				if (Interface != null && Interface.AddressFamily != Host.AddressFamily)
					throw new CharonException("Interface '" + Interface + "' is not compatible with the address family for Host '" + Host + "'.");

				if (_multicast && Protocol != Proto.Udp)
					throw new CharonException("Multicast hosts are not supported with the tcp protocol.");
			}
		}
			...
			if (_multicast)
			{
				if (Platform.GetOS() == Platform.OS.OSX)
				{
					if (local.Equals(IPAddress.Any) || local.Equals(IPAddress.IPv6Any))
						throw new CharonException("Error, the value for parameter 'Interface' can not be '" + local + "' when the 'Host' parameter is multicast.");
				}
