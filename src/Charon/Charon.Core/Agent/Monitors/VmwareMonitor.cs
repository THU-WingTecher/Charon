...
namespace Charon.Core.Agent.Monitors
{
	...
	public class VmwareMonitor : Monitor
	{
		...
		private class VMwareException : CharonException
		{
			...
			: base(agent, name, args)
		{
			...
			if (!SnapshotIndex.HasValue && string.IsNullOrEmpty(SnapshotName))
				throw new CharonException("Either SnapshotIndex or SnapshotName is required.");

			if (SnapshotIndex.HasValue && !string.IsNullOrEmpty(SnapshotName))
				throw new CharonException("Only specify SnapshotIndex or SnapshotName, not both.");
			...
			catch (DllNotFoundException ex)
			{
				...
				throw new CharonException(msg, ex);
			}
		}
		...
		protected void OpenHandle()
		{
			...
			catch (VMwareException ve)
			{
				...
				throw new CharonException(msg, ve);
			}
		}
