...
{
	/// <summary>
	/// Create an XPath Navigator for Charon DOM objects.
	/// </summary>
	/// <remarks>
	/// The XPath query syntax is the purfect way to select nodes
	/// from a Charon DOM.  By implementing an XPathNavigator we 
	...
	/// </remarks>
	public class CharonXPathNavigator : XPathNavigator
	{
		...
		/// <summary>
		/// Map between Type and CharonXPathNodeType
		/// </summary>
		protected static Dictionary<Type, CharonXPathNodeType> NodeTypeMap = new Dictionary<Type, CharonXPathNodeType>();

		/// <summary>
		/// The Charon DOM we are navigating.
		/// </summary>
		public Dom dom;
		...
		/// </summary>
		public CharonXPathNodeType currentNodeType;
		...
		static CharonXPathNavigator()
		{
			...
			NodeTypeMap[typeof(Dom)] = CharonXPathNodeType.Root;
			NodeTypeMap[typeof(DataElement)] = CharonXPathNodeType.DataModel;
			NodeTypeMap[typeof(StateModel)] = CharonXPathNodeType.StateModel;
			NodeTypeMap[typeof(State)] = CharonXPathNodeType.StateModel;
			NodeTypeMap[typeof(Action)] = CharonXPathNodeType.StateModel;
			NodeTypeMap[typeof(Test)] = CharonXPathNodeType.Test;
		}

		protected CharonXPathNodeType MapObjectToNodeType(object obj)
		{
		...
		public CharonXPathNavigator(Dom dom)
		{
			currentNode = dom;
			currentNodeType = CharonXPathNodeType.Root;
		}

		protected CharonXPathNavigator(Dom dom, object currentNode, CharonXPathNodeType currentNodeType, 
		...
		{
			return new CharonXPathNavigator(dom, currentNode, currentNodeType, attributeIndex, iteratingAttributes);
		}
		...
		{
			if (!(other is CharonXPathNavigator))
				return false;

			var otherXpath = other as CharonXPathNavigator;
		...
		public override bool MoveToFirstChild()
		{
			...
			else if (currentNode is Dom)
			{
				...
				if (dom.tests.Count > 0)
				{
					...
					currentNodeType = CharonXPathNodeType.Test;
					return true;
				}
			...
			else if (currentNode is Action)
			{
				...
				if (action.dataModel != null)
				{
					...
					currentNodeType = CharonXPathNodeType.DataModel;
					return true;
				}
			...
			else if (currentNode is Test)
			{
				...
				currentNodeType = CharonXPathNodeType.StateModel;
				return true;
			}
		...
		public override bool MoveToNext()
		{
			...
			if (currentNodeType == CharonXPathNodeType.Root)
				return false;
