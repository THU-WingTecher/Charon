...
namespace Charon.Core
{
	...
	public static class XmlExtensions
	{
		...
		public static string getAttrString(this XmlNode node, string name)
		{
			string value = node.getAttr(name, null);
			if (value == null)
				throw new CharonException("Error, '" + node.Name + "' element is missing required attribute '" + name + "'.");
			return value;
		}
		...
		private static bool StringToBool(XmlNode node, string name, string value)
		{
			switch (value)
			{
				...
				case "false":
					return false;
				default:
					throw new CharonException(getError(node, name) + "  Could not convert value '" + value + "' to a boolean.");
			}
		}
		...
		{
			if (value.Length != 1)
				throw new CharonException(getError(node, name) + "  Could not convert value '" + value + "' to a character.");
			return value[0];
		}
		...
		private static int StringToInt(XmlNode node, string name, string value)
		{
			...
			if (!int.TryParse(value, out ret))
				throw new CharonException(getError(node, name) + "  Could not convert value '" + value + "' to an integer.");
			return ret;
		}
