			...
			: base(args)
		{
			...
			try
			{
				...
				if (FileName == fileTemplate)
					throw new CharonException("Error, FileName \"" + fileTemplate + "\" missing iteration format identifier.");

				FileName = null;
			}
			catch (FormatException ex)
			{
				throw new CharonException("Error, FileName \"" + fileTemplate + "\" is not a valid format string.", ex);
			}
		}
