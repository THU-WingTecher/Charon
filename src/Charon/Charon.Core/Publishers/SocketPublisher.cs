...
namespace Charon.Core.Publishers
{
	public abstract class SocketPublisher : Publisher
	{
		...
		void JoinGroupV6(IPAddress group, uint ifindex)
		{
			...
			if (ret != 0)
				throw new CharonException("Error, failed to join group '{0}' on interface '{1}', error {2}.".Fmt(group, _iface, ret));

			ret = setsockopt(_socket.Handle.ToInt32(), IPPROTO_IPV6, IPV6_MULTICAST_IF, ref ptr, Marshal.SizeOf(typeof(IntPtr)));
			if (ret != 0)
				throw new CharonException("Error, failed to set outgoing interface to '{1}' for group '{0}', error {2}.".Fmt(group, _iface, ret));
		...
		private IPEndPoint ResolveHost()
		{
			...
			throw new CharonException("Could not resolve the IP address of host \"" + Host + "\".");
		}
		...
		private static IPAddress GetScopeId(IPAddress ip)
		{
			...
			if (results.Count == 0)
				throw new CharonException("Could not resolve scope id for interface with address '" + ip + "'.");

			if (results.Count != 1)
				throw new CharonException(string.Format("Found multiple interfaces with address '{0}'.{1}\t{2}",
		...
		private ProtocolType GetProtocolType(int protocol)
		{
			switch (protocol)
			{
				...
				default:
					throw new CharonException(string.Format("Error, the {0} publisher does not support protocol type 0x{1:X2}.", _type, protocol));
			}
		}
		...
		protected Socket OpenRawSocket(AddressFamily af, int protocol)
		{
			...
			throw new CharonException(_type + " publisher could not open raw socket.");
		}
		...
		protected override void OnStart()
		{
			...
			try
			{
				...
				if (!AddressFamilySupported(ep.AddressFamily))
					throw new CharonException(string.Format("The resolved IP '{0}' for host '{1}' is not compatible with the {2} publisher.", ep, Host, _type));
				...
				try
				{
					...
					if (iface == null)
						throw new CharonException("Could not resolve interface name for local IP '{0}'.".Fmt(local));
		...
		protected override void OnOpen()
		{
			...
			try
			{
				...
				if (_multicast)
				{
					...
					{
						if (_iface == null)
							throw new CharonException("Error, could not resolve local interface name for local IP '{0}'.".Fmt(_localIp));

						uint ifindex = if_nametoindex(_iface);

						if (ifindex == 0)
							throw new CharonException("Error, could not resolve interface index for interface name '{0}'.".Fmt(_iface));
					...
					{
						throw new CharonException(string.Format("Error, the value for parameter 'Interface' can not be '{0}' when the 'Host' parameter is multicast.", Interface == null ? "<null>" : Interface.ToString()));
					}
				}
				else
				{
			...
			catch (Exception ex)
			{
				...
				if (se != null && se.SocketErrorCode == SocketError.AccessDenied)
					throw new CharonException(string.Format("Access denied when trying open a {0} socket.  Ensure the user has the appropriate permissions.", _type), ex);
