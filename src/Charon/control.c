...
//用于has_new_bits函数的共享内存拷贝
/*
  由于has_new_bits需要传入classify之后的trace_bit
  trace_bit一次classify没有问题
  但是多次classify就会改变当前的trace_bits，造成不准
  所以如果有多次检测has_new_bits的需求，一定要使用cp_trace_bits去classify，保留原先的trace_bit
*/
static u8 cp_trace_bits[MAP_SIZE];

static u8 ini_trace_bits[MAP_SIZE];
...
//   //终止性检测，这里要注意先classify再传入
//   //同样，多次执行，需要传入拷贝的trace_bits
//   memcpy(cp_trace_bits, trace_bits, MAP_SIZE);
// #ifdef __x86_64__
//   classify_counts((u64*)cp_trace_bits);
// #else
//   classify_counts((u32*)cp_trace_bits);
// #endif /* ^__x86_64__ */

//   return has_new_bits(session_virgin_bits, cp_trace_bits);
}
...
{ 
  ...
  memcpy(cp_trace_bits, trace_bits, MAP_SIZE);
    ...
    u8 hnb = has_new_bits(virgin_bits, cp_trace_bits);
...
int charon_update(int iteration){
    printf("charon:charon_update start\n");
    memcpy(virgin_bits_maintain,virgin_bits,MAP_SIZE);
    iteration_maintain=iteration;
    printf("charon:charon_update end\n");
    return 0;
}

int charon_save(char * path){
    FILE *fp= fopen(path,"wb");
    if(fp==NULL){
        printf("charon:charon_save function fopen error!\n");
        return 1;
    }
    if(1 != fwrite(&iteration_maintain,sizeof(iteration_maintain),1,fp)){
        printf("charon:charon_save function writes error iteration!\n");
        return 1;
    }
    int return_byte_num = fwrite(virgin_bits_maintain,1,MAP_SIZE,fp);
    if(return_byte_num != MAP_SIZE){
        printf("charon:charon_save function writes error virgin_bit!\n");
        return 1;
    }
    fclose(fp);
    return 0;
...
int charon_read(char *path,int * iteration){
    FILE * fp = fopen(path,"rb");
    if(fp==NULL){
        printf("charon:charon_read function cannot open the virgin file!\n");
        return 1;
    }
    if(1 != fread(iteration,sizeof(iteration_maintain),1,fp)){
        printf("charon:charon_read function reads error iteration!\n");
        return 1;
    }
    if(MAP_SIZE != fread(virgin_bits,1,MAP_SIZE,fp)){
        printf("charon:charon_read function reads error virgin_bit!\n");
        return 1;
    }
    fclose(fp);
    return 0;
